...
class PID_controller:
    def __init__(self , debug = False):
        ...
        self.twist_pub = rospy.Publisher('/junkbot/cmd_vel', Twist, queue_size=10)
	...
	blurred = cv2.GaussianBlur(gray, (7, 7), 0)
        ...
        for c in cnts:
		    ...
		    if self.debug:	
                    ...
                    self.testPID(frame,translation_vector,1.0, 1.0, 0)

        # draw the status text on the frame
        if self.debug:
		cv2.putText(frame, status, (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 0), 2)
    ...
    def testPID(self,frame, translation_vector,P=0.4, I=0.0, D=0.1, L=10):
        ...
        setpoint = self.pid.SetPoints(int(center_x))
        ...
        for i in range(1, End):
            ...
            feed = self.pid.update(feedback)
            print feed, "feed"
            if (int(320 + feed) > lf) and (int(320 + feed) < lr):
                print("<<<<<<--------Go Straight------>>>>>")
                status = ">>>>>>>"
            elif feed > 0:
                print ("right------>>>>>")
            ...
            self.drive_robot(translation_vector[2] , feed)
    ...
    def drive_robot(self, trans, angle):
        self.twist.linear.x = self.translate( trans,-100, 100, -.5, .5)
        self.twist.linear.y = 0
        self.twist.linear.z = 0
        self.twist.angular.x = 0
        self.twist.angular.y = 0
        self.twist.angular.z = self.translate(angle, -60, 60, -.35, .35)
